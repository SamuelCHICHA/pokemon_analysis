...
```{r Première comparaison, echo=FALSE}
...
```{r Deuxième comparaison, echo = FALSE}
#Getting pokemons of the same generation and same type than the one we picked
same_type_pokemons <- same_gen_pokemons %>% filter(same_gen_pokemons$`Type 1` == pokemon$`Type 1`) #Better ?
if(!is.na(pokemon$`Type 2`))
  same_type_pokemons <- same_type_pokemons %>% filter(same_gen_pokemons$`Type 2` == pokemon$`Type 2`)
...
```{r Recherche pokémon pouvant battre Lugia, echo = FALSE}
lugia <- pokemons %>% filter(Name == 'Lugia')
non_leg_pokemons <- pokemons %>% filter(Legendary == FALSE)
fast_pok <- non_leg_pokemons %>% filter(Speed >= lugia$Speed)
off_pok <- non_leg_pokemons %>% filter(Attack >= lugia$Attack)
def_pok <- non_leg_pokemons %>% filter(Defense >= lugia$Defense)
hp_pok <- non_leg_pokemons %>% filter(HP >= lugia$HP)
sp_off_pok <- non_leg_pokemons %>% filter(non_leg_pokemons$'Sp. Atk' >= lugia$`Sp. Atk`)
sp_def_pok <- non_leg_pokemons %>% filter(non_leg_pokemons$'Sp. Def' >= lugia$`Sp. Def`)
