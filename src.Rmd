...
```{r Introduction to the database, 1st}
...
Nous étudierons ici le pokémon nommé __`r pokemon$Name`__.
...
On veut s'intéresser maintenant un peu plus aux données et comment celles-ci sont répartis. Pour cela, on va utiliser deux graphiques différents pour représenter cette répartition, une _courbe de densité_ et une _boîte à moustache_.
...
grid.arrange(attack_density, defense_density, speed_density, hp_density, sp_attack_density, sp_defense_density)
...
grid.arrange(attack_box, defense_box, speed_box, hp_box, sp_attack_box, sp_defense_box)

```

Ici, on peut constater une certaine asymétrie à gauche sur toutes les "bôites à moustaches" ainsi que sur les courbes de densité. On peut également voir un grand nombre __d'aberrations__ sur les données représentant les points de vie des pokémons. 

Nos données ne sont donc pas très bien réparties.
...
En regardant le graphique on peut constater que `r pokemon$Name` a `r ifelse(result[pokemon$Name, "HP"] > result["Moyenne", "HP"], "plus", "moins")` de points de vie que la moyenne des autres pokémons de sa génération.

Regardons maintenant pour ses autres caractéristiques :

* `r ifelse(result[pokemon$Name, "Speed"] > result["Moyenne", "Speed"], "Plus", "Moins")` de vitesse que la moyenne. 
* `r ifelse(result[pokemon$Name, "Attack"] > result["Moyenne", "Attack"], "Plus", "Moins")` d'attaque que la moyenne.
* `r ifelse(result[pokemon$Name, "Defense"] > result["Moyenne", "Defense"], "Plus", "Moins")` de défense que la moyenne.
* `r ifelse(result[pokemon$Name, "Sp..Atk"] > result["Moyenne", "Sp..Atk"], "Plus", "Moins")` d'attaque spéciale que la moyenne.
* `r ifelse(result[pokemon$Name, "Sp..Def"] > result["Moyenne", "Sp..Def"], "Plus", "Moins")` de défense spéciale que la moyenne.

Maintenant que nous avons placé `r pokemon$Name` par rapport à la moyenne des pokémons de sa génération, essayons désormais de le classer en faisant un top.
